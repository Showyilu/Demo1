  ...
  describe ".decode" do
    ...
    context "with an invalid token" do
      context "that is junk" do
        ...
        it "raises exception saying 'Not enough or too many segments'" do
          expect { decoded_token }.to raise_error(
            JWT::DecodeError,
      ...
      context "that has been fiddled with" do
        let(:encoded_token) do
          described_class.new(secret).encoded.tap { |token| token[0] = "Modified token" }
        end
      ...
      context "that is expired" do
        ...
        it "raises exception saying 'Signature has expired'" do
          Timecop.freeze(Time.zone.now + 120) do
    ...
    context "with a valid token" do
      let(:encoded_token) do
        ...
        hmac_token.expire_time = Time.zone.now + expire_time
        hmac_token.encoded
      end
